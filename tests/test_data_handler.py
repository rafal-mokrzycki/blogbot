...
def test_transform_correct_start(input_string):
    string = Transformer.transform(
        input_string, temperature=0.6, max_length=100, n_sentences=1
    )[0]["generated_text"]
...
def test_transform_correct_length(input_string):
    string = Transformer.transform(
        input_string, temperature=0.0001, max_length=100, n_sentences=1
    )[0]["generated_text"]
    assert len(string.split(" ")) == 81


def test_transform_correct_n_sentences(input_string):
    sentences = Transformer.transform(
        input_string, temperature=0.6, max_length=100, n_sentences=5
    )
